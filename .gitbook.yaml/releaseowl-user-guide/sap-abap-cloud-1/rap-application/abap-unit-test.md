---
hidden: true
---

# ABAP Unit Test

The ABAP Unit Test Report displays the results of automated unit tests executed during the Build process. It validates the functional correctness of the transported ABAP objects by executing associated test classes and methods. If ABAP Unit Test Checks are enabled in the Build Pipeline, the report is available in the Build execution screen.

#### To View the AUnit Report

1. Go to the created **Build Pipeline**.
2. Open the completed Build execution.

<figure><img src="../../../.gitbook/assets/image (1695).png" alt=""><figcaption></figcaption></figure>

3. Click on **ABAP  Report** to view the detailed static code analysis results.

<figure><img src="../../../.gitbook/assets/image (1692).png" alt=""><figcaption></figcaption></figure>
